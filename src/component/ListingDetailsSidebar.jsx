...
const ListingDetailsSidebar = ({ isOpen, onClose, listing }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className="fixed inset-y-0 right-0 w-full sm:w-[450px] md:w-[500px] bg-white shadow-2xl z-[101] overflow-y-auto transform transition-transform duration-300 flex flex-col rounded-l-[2rem]">
        <div className="p-8 flex-1">
          ...
          <div className="flex gap-4 overflow-x-auto mb-8 pb-2 hide-scrollbar">
            <div className="w-48 h-48 flex-shrink-0 bg-[#f8f9fa] rounded-3xl overflow-hidden relative border border-gray-50 p-4">
              <img src={listing.image || 'https://via.placeholder.com/200'} alt={listing.title} className="w-full h-full object-contain mix-blend-multiply" />
            </div>
            {/* Show a duplicate image for illustration purposes as in the design */}
            <div className="w-48 h-48 flex-shrink-0 bg-[#f8f9fa] rounded-3xl overflow-hidden relative border border-gray-50 p-4">
              <img src={listing.image || 'https://via.placeholder.com/200'} alt={listing.title} className="w-full h-full object-contain mix-blend-multiply" />
            </div>
          </div>
          ...
          <div className="border border-dashed border-gray-300 rounded-[2rem] p-8 mb-8">
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
              <div>
                ...
                <p className="text-sm font-bold text-gray-900">{listing.category?.split(' • ')[0] || 'Goods'}</p>
              </div>
              <div>
                <h4 className="text-[10px] font-bold text-gray-500 mb-2 uppercase tracking-wider">Sub-category</h4>
                <p className="text-sm font-bold text-gray-900">{listing.category?.split(' • ')[1] || 'Gadgets'}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-gray-900">{listing.duration || '30 Days'}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className={`rounded-3xl p-6 text-center ${
              (listing.status || 'Active') === 'Active' ? 'bg-[#e6f0ff] text-blue-600' : 'bg-[#e6ffe6] text-green-600'
            }`}>
